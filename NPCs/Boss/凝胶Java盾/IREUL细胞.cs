...
namespace Entrogic.NPCs.Boss.凝胶Java盾
{
    ...
    public class IREUL细胞 : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (MyTile != null)
            {
                ...
                {
                    if (n.active && n.Center.X >= npc.Center.X - 25 && n.Center.X <= npc.Center.X + 25 && n.Center.Y >= npc.Center.Y - 25 && n.Center.Y <= npc.Center.Y + 25 && n.type == NPCType<IREUL细胞2>())
                    {
                        count++;
                    }
                }
                ...
                if (npc.ai[0] >= (Main.expertMode ? 40 : 60))
                {
                    if (Main.rand.NextBool(2) && Main.tile[tileMe.X + 1, tileMe.Y].active() && (Main.tileSolid[Main.tile[tileMe.X + 1, tileMe.Y].type] || Main.tileSolidTop[Main.tile[tileMe.X + 1, tileMe.Y].type]))
                        NPC.NewNPC(0, 0, NPCType<IREUL细胞2>(), 0, MyPosition.X + 16, MyPosition.Y);
                    if (Main.rand.NextBool(2) && Main.tile[tileMe.X - 1, tileMe.Y].active() && (Main.tileSolid[Main.tile[tileMe.X - 1, tileMe.Y].type] || Main.tileSolidTop[Main.tile[tileMe.X - 1, tileMe.Y].type]))
                        NPC.NewNPC(0, 0, NPCType<IREUL细胞2>(), 0, MyPosition.X - 16, MyPosition.Y);
                    if (Main.rand.NextBool(2) && Main.tile[tileMe.X, tileMe.Y + 1].active() && (Main.tileSolid[Main.tile[tileMe.X, tileMe.Y + 1].type] || Main.tileSolidTop[Main.tile[tileMe.X, tileMe.Y + 1].type]))
                        NPC.NewNPC(0, 0, NPCType<IREUL细胞2>(), 0, MyPosition.X, MyPosition.Y + 16);
                    if (Main.rand.NextBool(2) && Main.tile[tileMe.X, tileMe.Y - 1].active() && (Main.tileSolid[Main.tile[tileMe.X, tileMe.Y - 1].type] || Main.tileSolidTop[Main.tile[tileMe.X, tileMe.Y - 1].type]))
                        NPC.NewNPC(0, 0, NPCType<IREUL细胞2>(), 0, MyPosition.X, MyPosition.Y - 16);
            ...
            foreach (NPC n in Main.npc)
            {
                if (n.active && n.type == NPCType<嘉沃顿>())
                {
                    Java++;
                }
            }
    ...
    public class IREUL细胞2 : ModNPC
    {
        public override string Texture { get { return "Entrogic/NPCs/Boss/凝胶Java盾/IREUL细胞"; } }
        public override void SetDefaults()
        {
            npc.width = 10;
            npc.height = 10;
        ...
        public override void AI()
        {
            ...
            foreach (NPC n in Main.npc)
            {
                ...
                if (n.active && (n.type == NPCType<IREUL细胞2>() || n.type == NPCType<IREUL细胞>()))
                {
                    count2++;
                }
                if (n.active && n.type == NPCType<嘉沃顿>())
