...
namespace Entrogic.Common.ModSystems
{
    internal class SpookyLampHandler : ModSystem
	{
		...
		internal static List<Point> SpookyLamps = new List<Point>();
		internal static List<int> Lamps = new List<int>(); // 所有注册的诡异台灯都在这里
		...
		internal static void HandleLamps(Player player, bool active = true) {
			...
			for (int x = -5; x <= 5; x++) {
				for (int y = -5; y <= 5; y++) {
					Point pt = new Point(pl.X + x, pl.Y + y);
					Tile t = Framing.GetTileSafely(pt);
					if (Vector2.Distance(pl.ToVector2(), pt.ToVector2()) <= 4 && IsSpookyLamp(t.type)) { // 足够近
						SpookyLamps.Add(pt); // 添加进集合
						if (t.frameX == 18) { // 如果是关着的那顺便开起来
			...
			foreach (var p in from pt in SpookyLamps
							  where (Vector2.Distance(pl.ToVector2(), pt.ToVector2()) > 4 || !active) && Framing.GetTileSafely(pt).frameX == 0 // 开启状态，足够远或玩家消失
							  select pt) {
				if (IsSpookyLamp(Framing.GetTileSafely(p).type)) // 有可能台灯被挖掉，所以要给个特判了
