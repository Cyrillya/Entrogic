...
namespace Entrogic.Content.Contaminated.Weapons
{
    public class ContyCurrent : ItemBase
    {
        ...
        private void Player_ItemCheck_Shoot(On.Terraria.Player.orig_ItemCheck_Shoot orig, Player self, int i, Item sItem, int weaponDamage) {
            if (sItem.type == Type) {
                ...
                for (int p = 0; p < Main.maxProjectiles; p++) {
                    if (Main.projectile[p].active && Main.projectile[p].type == ModContent.ProjectileType<ContyCurrentProj2>() && Main.projectile[p].owner == self.whoAmI) {
        ...
        public override void SetDefaults() {
            ...
            Item.autoReuse = true;
            Item.shoot = ModContent.ProjectileType<ContyCurrentProj2>();
            Item.shootSpeed = 24f;
        }
        ...
        public override void ModifyShootStats(Player player, ref Vector2 position, ref Vector2 velocity, ref int type, ref int damage, ref float knockback) {
            type = ModContent.ProjectileType<ContyCurrentProj>();
            for (int p = 0; p < Main.maxProjectiles; p++) {
                if (Main.projectile[p].active && Main.projectile[p].type == ModContent.ProjectileType<ContyCurrentProj2>() && Main.projectile[p].owner == player.whoAmI) {
