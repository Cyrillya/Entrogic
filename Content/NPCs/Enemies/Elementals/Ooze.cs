...
{
    public class Ooze : NPCBase, IModifyCollisionData
    {
        ...
        public override void AI() {
            ...
            bool hasHitablePlayer = Player.Exists() && Collision.CanHitLine(NPC.position, NPC.width, NPC.height, targetPosition, Player.width, Player.height);
            if (hasHitablePlayer && distance <= 160 && JumpCount <= 5) {// 硬核保持距离
            ...
            if (NPC.velocity.Y == 0f) {
                ...
                // 如果能直线看到玩家，就向玩家去
                if (JumpCount <= 5 && Collision.CanHitLine(NPC.position, NPC.width, NPC.height, targetPosition, Player.width, Player.height)) {
                    int toPlayerYTile = (int)(targetCenter.Y - NPC.Center.Y) >> 4;
                    int distanceXTile = Math.Abs((int)(targetCenter.X - NPC.Center.X) >> 4);
                    if (distanceXTile <= 6 && toPlayerYTile < -2) { // 玩家离得近，高度比史莱姆高
        ...
        public bool ModifyCollisionData(NPC npc, Rectangle victimHitbox, ref float damageMultiplier, ref Rectangle npcHitbox) {
            if (State == 1 && (Timer / 5 % 6) >= 4) {
