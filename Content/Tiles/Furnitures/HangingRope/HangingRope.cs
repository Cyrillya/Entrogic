...
namespace Entrogic.Content.Tiles.Furnitures.HangingRope
{
	...
	internal class HangingRope_Tile : TileBase
	{
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY) => Item.NewItem(i * 16, j * 16, 64, 32, ModContent.ItemType<HangingRope>());

		public override bool RightClick(int i, int j) {
			Player player = Main.LocalPlayer;

			Tile tile = Main.tile[i, j];
			int topX = i - tile.frameX / 18 % 2;
