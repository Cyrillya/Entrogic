...
public class ModImageButton : UIElement
{
    ...
    protected override void DrawSelf(SpriteBatch spriteBatch) {
        ...
        if (BackgroundTexture is not null) {
            spriteBatch.Draw(BackgroundTexture.Value, dimensions.Center(), null, mainColor, 0f,
                BackgroundTexture.Size() / 2f, 1f, SpriteEffects.None, 0f);
        }

        spriteBatch.Draw(Texture.Value, dimensions.Center(), null, mainColor, 0f, Texture.Size() / 2f, 1f,
            SpriteEffects.None, 0f);
        if (BorderTexture is not null && IsMouseHovering) {
            spriteBatch.Draw(BorderTexture.Value, dimensions.Center(), null, mainColor * 1.4f, 0f,
                BorderTexture.Size() / 2f, 1f, SpriteEffects.None, 0f);
        }
    }
